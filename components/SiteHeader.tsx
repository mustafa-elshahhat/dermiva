    ...
    <>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(253,246,244,.92)", backdropFilter: "blur(10px)", borderBottom: "1px solid #f0dde1" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", width: "100%", padding: "0 clamp(16px,4vw,40px)", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <button aria-label="Open menu" onClick={() => setMenuOpen(true)} className="dm-burger" style={{ background: "none", border: "none", cursor: "pointer", flexDirection: "column", gap: 4, padding: 6 }}>
          ...
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
            <Image
              ...
              priority
            />
            <span className="dm-serif" style={{ fontWeight: 700, fontSize: 25, letterSpacing: ".22em", color: "#c0934a" }}>DERMIVA</span>
          </Link>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <button aria-label="Account" title="Account" className="dm-icon-btn" onClick={() => router.push(accountHref)}>
              <AccountIcon />
            </button>
            <button aria-label="Wishlist" title="Wishlist" className="dm-icon-btn" onClick={() => router.push("/wishlist")}>
              <HeartIcon />
