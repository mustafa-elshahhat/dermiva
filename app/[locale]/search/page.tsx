...
import { CATS, searchProducts, getCategoryProduct, productImage, type CategoryKey } from "@/lib/catalog";
            ...
            {catKeys.map((k) => {
              ...
              <Link key={k} href={`/category/${k}`} style={{ cursor: "pointer", background: "#fff", border: "1px solid #f0dde1", borderRadius: 16, overflow: "hidden", textAlign: "center", display: "block" }}>
                <div style={{ aspectRatio: "1/1", overflow: "hidden" }}>
                  <ProductImage image={productImage(catProduct)} mode="packshot" name={label} kind={catProduct.kind} style={{ objectFit: "cover" }} />
                </div>
                <div className="dm-serif" style={{ fontWeight: 600, fontSize: 16, color: "#4f3a3e", padding: "10px 12px 14px" }}>{label}</div>
              </Link>
              );
            })}
          </div>
