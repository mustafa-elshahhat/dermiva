...
export default function AddressesPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formName.trim() || !formAddress.trim() || !formCity.trim() || !formGov.trim() || !formPhone.trim()) {
      setFormError(t("account.addressesError"));
      showToast("fieldsIncomplete");
      return;
    }
    if (!/^[0-9+\-\s]{8,}$/.test(formPhone)) {
      setFormError(t("account.addressesError"));
      showToast("invalidPhone");
      return;
    }
  ...
  return (
    <div className="dm-fade" style={{ maxWidth: 800, margin: "0 auto", width: "100%", padding: "clamp(20px,3vw,32px) clamp(16px,4vw,40px) clamp(40px,5vw,64px)" }}>
      <div style={{ marginBottom: 8, fontSize: 12.5, color: "#a98e93" }}>
        <Link href="/account" style={{ textDecoration: "underline" }}>{t("seo.accountTitle")}</Link> / <span style={{ color: "#7c6065" }}>{t("account.addressesBreadcrumb")}</span>
      </div>

      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "baseline", gap: 12, marginBottom: 22 }}>
        <h1 className="dm-serif" style={{ fontWeight: 700, fontSize: "clamp(32px,4.5vw,46px)", color: "#5a4145", margin: 0 }}>{t("account.addressesTitle")}</h1>
        {!isEditing ? (
          <button onClick={handleAddNew} className="dm-btn-primary" style={{ padding: "10px 22px", fontSize: 13, textTransform: "uppercase" }}>{t("account.addressesAddNew")}</button>
        ) : null}
      </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #f0dde1", borderRadius: 20, padding: 24 }} className="dm-fade">
          ...
          <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "6px 0" }}>
              ...
              />
              <label htmlFor="isDefault" style={{ fontSize: 13.5, color: "#7c6065", cursor: "pointer" }}>{t("account.addressesSetDefault")}</label>
            </div>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
              <button type="submit" className="dm-btn-primary" style={{ padding: "12px 28px", fontSize: 13, textTransform: "uppercase" }}>{t("account.addressesSave")}</button>
              <button type="button" onClick={() => setIsEditing(false)} className="dm-btn-outline" style={{ padding: "11px 28px", fontSize: 13, textTransform: "uppercase" }}>{t("account.addressesCancel")}</button>
            </div>
          </form>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {addresses.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", background: "#fff", borderRadius: 20, border: "1px solid #f0dde1" }}>
              <p style={{ fontSize: 14.5, color: "#a98e93", margin: "0 0 14px" }}>{t("account.addressesEmptyText")}</p>
              <button onClick={handleAddNew} className="dm-btn-primary" style={{ padding: "10px 24px" }}>{t("account.addressesAdd")}</button>
            </div>
          ) : (
                ...
                >
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
                      ...
                      {addr.isDefault ? (
                        <span style={{ fontSize: 10.5, background: "#faecef", color: "#b76e79", padding: "3px 9px", borderRadius: 999, fontWeight: 600 }}>{t("account.addressesDefault")}</span>
                      ) : null}
                    </div>
